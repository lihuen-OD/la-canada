...
## Actualización — Stock completo (Etapa 5C.2)

- **Reportes y CSV**: solo `GET`, detrás de `requireAuth`, con el mismo alcance para `ADMIN` y `EMPLOYEE` por paridad verificada con el prototipo. Schemas `.strict()`, rango ≤ 366 días y SQL parametrizado. El CSV tiene límite de filas, BOM UTF-8, escaping y neutralización de fórmulas; nunca expone PIN, tokens, hashes ni auditoría.
