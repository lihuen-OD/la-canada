...
- **Migración sin aplicar**: `20260924210000_stock_idempotency_balance_check` fue generada offline y revisada a mano (cero `DROP`/`TRUNCATE`/`DELETE`, cero `ON DELETE CASCADE`); aplicarla contra `demo` corresponde a 5C.1C con autorización. Sin secretos nuevos ni variables de entorno nuevas.
