...
| 6 | El saldo (`StockItem.currentQuantity`) nunca queda negativo tras `CONSUMPTION`/`ADJUSTMENT_DECREASE` | — | ✅ `CHECK (current_quantity >= 0)` — **Etapa 5C.1A, en la migración `20260924210000_stock_idempotency_balance_check` — aplicada a `demo` en 5C.1C** | ✅ | — | Depende del saldo concurrente — no expresable como un `CHECK` que reemplace la lógica; la Etapa 5A usa una actualización condicional atómica (`currentQuantity >= quantity`) dentro de la transacción (garantía principal frente a concurrencia) y el `CHECK` de la Etapa 5C.1A es el piso de defensa en profundidad contra cualquier otra vía de escritura |
...
### Etapa 5C.1A — `IdempotencyRecord` y CHECK de saldo no negativo (aplicada a `demo` en 5C.1C)

Migración `20260924210000_stock_idempotency_balance_check`, generada **offline** con `prisma migrate diff --from-schema <schema antes> --to-schema <schema ahora>` (comparación pura entre dos archivos, sin base de datos ni shadow DB) y revisada a mano. **Estado**: aplicada a `demo` el 2026-09-25 (Etapa 5C.1C) con `db:migrate:deploy`, tras verificar la precondición (0 saldos negativos); `production` no se tocó. Los comentarios de cabecera del `migration.sql` todavía dicen "NO fue aplicada": no se editan porque la migración ya está aplicada y modificarla alteraría su checksum en `_prisma_migrations`.
...
### Etapa 5X — Más: novedades, eventos y galería (aplicada a `demo`)

Migración `20260925200000_more_module` (offline con `prisma migrate diff`, revisada, solo agrega o cambia índices; no borra columnas ni filas):

- `news_reports.recorded_by_user_id` (FK `users`, nullable solo por las 2 filas del seed): actor real, separado de la persona que reporta. Se **quitó** el índice único `(employee_id, text)`: el prototipo permitía repetir un aviso; el seed ahora busca por esos campos con `findFirst`. Índice `(created_at)` para el historial.
- `events.deleted_at` / `deleted_by_user_id` (FK `users`) + `CHECK (deleted_at IS NULL) = (deleted_by_user_id IS NULL)`: el "✕" pasa a ser anulación lógica. La unicidad `(title, date, type)` pasa a ser **parcial** (`WHERE deleted_at IS NULL`). Índice `(date)`.
