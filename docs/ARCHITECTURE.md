...
## 22. Rendimiento, navegación y caché (Etapa 5P)

**Causas medidas** (Chrome real contra el build de producción, `/api` interceptado con datos sintéticos, 150 ms por request): `TasksSubnav` usaba `<a href>` → Tareas ↔ Desempeño recargaba el documento, reiniciaba React y repetía `refresh` + `/auth/me` ("Restaurando tu sesión", ~840 ms); cada pantalla guardaba sus datos en `useState` local y volvía a `loading` al montarse o al cambiar un filtro; el historial de Tareas esperaba a la lista (cascada); en desarrollo, StrictMode duplicaba GET y `/auth/me`.

**Navegación**: solo `Link`/`NavLink`/`useNavigate`. `AppProviders` (QueryClient → BrowserRouter → AuthProvider) se monta una vez por documento; el bootstrap de sesión ocurre solo al cargar/recargar el documento y es single-flight por intento (un `refresh` y un `/me`, también bajo StrictMode).

**Datos (TanStack Query)** — `frontend/src/api/queryClient.ts`, `queryKeys.ts`:
- Claves siempre con prefijo `['session', userId]`; la caché se cancela y vacía en login, logout y cambio de sesión. Solo memoria: nunca `localStorage`/`sessionStorage`/IndexedDB ni persistidores (test estático `noSensitiveStorage.test.ts`).
- `staleTime`: operativo 30 s (tareas, historial, inventario, usuarios), métricas 60 s (desempeño), catálogo 5 min (empleados asignables, categorías, destinos). `gcTime` 10 min. Sin revalidación por foco (evita requests a Render/Neon al cambiar de pestaña); sí al reconectar.
- Reintentos: nunca para `ApiError`; uno ante falla de red en GET; mutaciones jamás. El único reintento de un POST sigue siendo el central tras 401 + refresh de `httpClient`.
- `placeholderData: keepPreviousData` en listados con filtros: la vista anterior queda visible (atenuada, `.is-stale`) hasta que llega la nueva; cada respuesta se guarda bajo su clave, así que una respuesta vieja nunca pisa un filtro nuevo. "Cargar más" usa `fetchNextPage({ cancelRefetch: false })`.
- Invalidación selectiva: Tareas (crear/editar/completar/revertir/activar) → `tasks.*` + `performance.*`; movimiento de stock → `stock.items` + `stock.movements`; catálogo de stock → `stock.items` + `stock.categories`; usuarios → `admin.users`. Nunca un `invalidateQueries()` global.

**Estados de carga**: bootstrap global solo al restaurar la sesión; primera carga de un módulo = estado localizado dentro del contenido (el shell nunca se desmonta); revalidación = datos visibles + "Actualizando…" junto al título (`PageHeader refreshing`, espacio reservado, sin saltos, estático con reduced motion); mutación = solo el control/diálogo afectado; error de revalidación con datos = aviso con "Reintentar" sin borrar la lista.

**Sesión y red**: si el refresh falla por red o 5xx, `httpClient` propaga ese error (la pantalla ofrece reintentar) en vez de un 401 que cerraría la sesión; el bootstrap trata 5xx como `sessionError` reintentable. El logout limpia primero el estado local (token, época y caché) y después espera al refresh en vuelo antes de `POST /auth/logout`, para revocar la cookie más reciente.

**Backend**: `requireAuth` valida sesión + usuario + empleado en UNA sentencia SQL estática y parametrizada (`Prisma.sql`, JOIN por `sid`); antes eran 2 sentencias secuenciales en `/auth/me` y rutas admin, y 4 en las rutas de negocio contando `resolveActor`. Un `select` anidado de Prisma NO alcanza: el cliente lo ejecuta como una sentencia por relación (verificado contra Postgres real — `requireAuth.integration.test.ts` cuenta las sentencias que llegan a la base y falla si hay más de una), y `relationJoins` exigiría cambiar el schema. `resolveActor` reutiliza `req.auth.employeeId`. Cliente Prisma único con `DATABASE_URL` pooled; `DIRECT_URL` solo para migraciones. Dentro de una transacción interactiva las consultas son secuenciales (§21). Sin keep-alive artificial: el arranque en frío de Render/Neon es visible en la primera carga y no se oculta.
