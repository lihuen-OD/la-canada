...
- **Stock (Etapas 5A + 5B + 5C.1)**: `/api/v1/stock` expone catálogo e historial autenticados; `ADMIN` administra categorías/productos, destinos (alta/edición/inactivación, sin borrado físico) y registra ajustes; cualquier usuario autenticado registra ingresos/consumos. El frontend `/stock` usa filtros y paginación server-side, historial inmutable y catálogo administrativo sin hablar directo con la base. Saldo, movimiento y auditoría se escriben en el backend dentro de una transacción con actualización decimal atómica. La Etapa 5C.1 agrega el filtro `stockLevel` resuelto en Postgres, `stockLevel` calculado en el DTO, y el header opcional `Idempotency-Key` en la creación de movimientos (replay exacto, sin doble escritura). Ver `docs/ARCHITECTURE.md` §20–§21 y `docs/MIGRATION_PLAN.md` (Etapas 5A/5B/5C.1).
- **Stock completo (Etapa 5C.2, sin commit)**: `/stock` con subvistas 🏠 Casa, 🌿 Jardín, 🛒 Compras (derivada del stock mínimo, compartible y agrupable), 📊 Reportes (agregados y CSV server-side) y ⚙️ Catálogo (ADMIN, incluye destinos). El frontend usa el `stockLevel` del backend, un modal unificado de ingreso/consumo y `Idempotency-Key` en cada movimiento. Los destinos son opcionales en cualquier movimiento. Ver `docs/ARCHITECTURE.md` §23.
- **Rendimiento (Etapa 5P)**: navegación SPA sin recargas ni re-restauración de sesión, datos del frontend con TanStack Query (caché en memoria por usuario, revalidación en segundo plano, invalidación selectiva) y `requireAuth` con una única consulta por request. Reglas permanentes en `AGENTS.md` (regla 13) y `docs/ARCHITECTURE.md` §22.
- **Gallinero (Etapa 5G, sin commit)**: `/chicken-coop` con KPIs, gallinas activas (configuración inicial y altas/bajas solo ADMIN), registro de recolección, análisis por período e historial paginado; "eliminar" es una anulación auditada. Ver `docs/ARCHITECTURE.md` §24.
- **Mascotas (Etapa 5M)**: `/pets` con listado por tipo, ficha con KPIs, historial clínico (anulación auditada), gestión de tipos y foto en Object Storage privado vía el backend (requiere `OBJECT_STORAGE_*`). Ver `docs/ARCHITECTURE.md` §25.
- **☰ Más (Etapa 5X, sin commit)**: `/more` con 📝 Novedades, 📅 Eventos y cumpleaños calculados, 🌤️ Clima (Open-Meteo vía backend), 📸 Fotos (Object Storage privado), ⚙️ Configuración (ADMIN: personas, datos del equipo, calendario, seguridad) y 👤 Mi perfil. Usuarios se abre desde Configuración. Ver `docs/ARCHITECTURE.md` §26.
- **Todavía no hay dashboard** (ni real ni mock). Lo ya implementado en frontend comprende autenticación, usuarios, Tareas, Desempeño, Stock, Gallinero, Mascotas y Más.
- **El modelo de datos ya está migrado contra Neon, rama `demo` exclusivamente** (Etapa 3A, con ajustes de `Session`/`User` en 3B.1/3B.2): `backend/prisma/schema.prisma` (24 modelos, 11 enums), migraciones aplicadas, y el seed ya ejecutado dos veces (idempotencia confirmada). La migración de la Etapa 5C.1A (`idempotency_records` + `CHECK` de saldo no negativo) fue **aplicada a `demo` en la Etapa 5C.1C** (2026-09-25), con `db:migrate:deploy`. `production` no se toca en esta etapa — ver "Base de datos (Prisma + Neon)" más abajo.
...
El modelo de datos (`backend/prisma/schema.prisma`, 24 modelos, 11 enums — ver `docs/DATABASE.md`) ya está migrado contra Neon, rama `demo` exclusivamente (Etapa 3A, con ajustes posteriores en 3B.1/3B.2/4A/4B). La migración de la Etapa 5C.1A (`20260924210000_stock_idempotency_balance_check`) fue **aplicada a `demo` en la Etapa 5C.1C** (2026-09-25). `production` no se toca en esta etapa.
...
**Migración de la Etapa 5C.1A — aplicada a `demo` en 5C.1C** (`backend/prisma/migrations/20260924210000_stock_idempotency_balance_check/`): crea `idempotency_records` (índice único por actor+endpoint+clave, índice por `created_at`, FK `RESTRICT`) y agrega a mano `CHECK (current_quantity >= 0)` sobre `stock_items`. Generada offline comparando dos archivos de schema (`prisma migrate diff --from-schema/--to-schema`), revisada a mano (cero `DROP`/`TRUNCATE`/`DELETE`, cero `ON DELETE CASCADE`). Fue aplicada a `demo` el 2026-09-25 (Etapa 5C.1C) con `db:migrate:deploy`, tras verificar la precondición (0 saldos negativos); `production` no se tocó — ver `docs/DATABASE.md`, "Etapa 5C.1A" y `docs/MIGRATION_PLAN.md`, "Etapa 5C.1C".
