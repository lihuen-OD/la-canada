...
- **Stock (Etapas 5A + 5B)**: `/api/v1/stock` expone catálogo e historial autenticados; `ADMIN` administra categorías/productos y registra ajustes; cualquier usuario autenticado registra ingresos/consumos. El frontend `/stock` usa filtros y paginación server-side, historial inmutable y catálogo administrativo sin hablar directo con la base. Saldo, movimiento y auditoría se escriben en el backend dentro de una transacción con actualización decimal atómica. Ver `docs/ARCHITECTURE.md` §20 y `docs/MIGRATION_PLAN.md` (Etapas 5A/5B).
