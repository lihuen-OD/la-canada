...
- **El modelo de datos ya está migrado contra Neon, rama `demo` exclusivamente** (Etapa 3A, con ajustes de `Session`/`User` en 3B.1/3B.2): `backend/prisma/schema.prisma` (24 modelos, 11 enums), migraciones aplicadas, y el seed ya ejecutado dos veces (idempotencia confirmada). La migración de la Etapa 5C.1A (`idempotency_records` + `CHECK` de saldo no negativo) está generada en el repositorio pero **sin aplicar** — pendiente de 5C.1C. `production` no se toca en esta etapa — ver "Base de datos (Prisma + Neon)" más abajo.
...
El modelo de datos (`backend/prisma/schema.prisma`, 24 modelos, 11 enums — ver `docs/DATABASE.md`) ya está migrado contra Neon, rama `demo` exclusivamente (Etapa 3A, con ajustes posteriores en 3B.1/3B.2/4A/4B). La migración de la Etapa 5C.1A (`20260924210000_stock_idempotency_balance_check`) está generada y revisada, pero **sin aplicar** — pendiente de 5C.1C. `production` no se toca en esta etapa.
...
**Migración de la Etapa 5C.1A — generada, sin aplicar** (`backend/prisma/migrations/20260924210000_stock_idempotency_balance_check/`): crea `idempotency_records` (índice único por actor+endpoint+clave, índice por `created_at`, FK `RESTRICT`) y agrega a mano `CHECK (current_quantity >= 0)` sobre `stock_items`. Generada offline comparando dos archivos de schema (`prisma migrate diff --from-schema/--to-schema`), revisada a mano (cero `DROP`/`TRUNCATE`/`DELETE`, cero `ON DELETE CASCADE`). Su aplicación con `db:migrate:deploy` contra `demo` corresponde a la Etapa 5C.1C, con autorización humana — ver `docs/DATABASE.md`, "Etapa 5C.1A".
