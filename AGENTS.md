...
**Actualización Etapa 5X (sin commit, lista para revisión visual):** ☰ Más full stack en `/more` — 📝 Novedades, 📅 Eventos (cumpleaños calculados, nunca filas), 🌤️ Clima (vía backend), 📸 Fotos (Object Storage privado; eliminar solo ADMIN), ⚙️ Configuración (personas, datos del equipo, calendario, seguridad) y 👤 Mi perfil. "Más" reemplaza a "Usuarios" en la barra (orden del prototipo); Usuarios se abre desde Configuración. Migración `20260925200000_more_module` aplicada **solo a `demo`**. Ver `docs/MIGRATION_PLAN.md`, "Etapa 5X", y `docs/ARCHITECTURE.md` §26.

**Actualización Etapa 5M (commit `814b216`, publicado):** 🐾 Mascotas full stack en `/pets` (listado, ficha con KPIs, registros clínicos con anulación lógica, gestión de tipos, foto por Neon Object Storage vía backend). Migración `20260925180000_pets_medical_voiding` aplicada **solo a `demo`**. Las fotos requieren configurar `OBJECT_STORAGE_*` (vacías en local: sin ellas, 503 claro y el resto funciona). Ver `docs/MIGRATION_PLAN.md`, "Etapa 5M", y `docs/ARCHITECTURE.md` §25. No iniciar Más sin autorización.

**Actualización Etapa 5G (commit local `6069df1`):** 🐔 Gallinero full stack en `/chicken-coop` — KPIs y análisis por período calculados en el backend (`BUSINESS_TIME_ZONE`), configuración inicial real por el ADMIN (sin sembrar ni inventar cantidades), altas/bajas condicionadas a la cantidad confirmada, recolecciones con persona según rol y actor real separado, `Idempotency-Key`, historial paginado por días y "eliminar" como anulación lógica auditada. Migración `20260925150000_chicken_coop_voiding_checks` aplicada **solo a `demo`**. Ver `docs/MIGRATION_PLAN.md`, "Etapa 5G", y `docs/ARCHITECTURE.md` §24. No avanzar a Mascotas sin autorización.
