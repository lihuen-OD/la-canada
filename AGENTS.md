...
Ver `docs/MIGRATION_PLAN.md` para la etapa vigente. El proyecto completó las Etapas 0 (documentación), 1 (estructura profesional), 2/2.1/2.2 (modelo de datos, seed real, Object Storage), 2.3 (retiro seguro del prototipo heredado — ver regla 9), 3A (conexión a Neon `demo`, migración inicial, seed real), 3B.1 (autenticación backend profesional: login, refresh con rotación, autorización por rol, administración de usuarios, bootstrap del primer admin construido y testeado pero nunca ejecutado), 3B.2 (corrección del modelo de credenciales: usuario+contraseña reemplazado por selección de identidad + PIN numérico individual de 4 dígitos, con bloqueo persistente por intentos fallidos), 3C (frontend completo del flujo de login por PIN — selector de identidad, teclado numérico, access token solo en memoria, restauración de sesión, cierre de sesión — conectado al backend real) y 3D (módulo de administración de usuarios en el frontend — `/admin/users`, exclusivo de `ADMIN`: listado real, activación con primer PIN, cambio de PIN, cambio de estado con protección de auto-bloqueo, revocación de sesión propia al cambiar el propio PIN o auto-suspenderse). `production` sigue sin tocarse; **el bootstrap del primer admin sigue sin ejecutarse** (preparado, pendiente de que el usuario lo corra personalmente — ver `docs/MIGRATION_PLAN.md`, "Etapa 3D"); los 4 empleados reales siguen `PENDING_ACTIVATION`, sin PIN, y no se activan ni se les asigna PIN sin autorización explícita puntual. Sin dashboard ni módulos de negocio todavía (real ni mock) — la Etapa 4 sigue pendiente.
