...
Ver `docs/MIGRATION_PLAN.md` para la etapa vigente. El proyecto completó las Etapas 0 (documentación), 1 (estructura profesional), 2/2.1/2.2 (modelo de datos, seed real, Object Storage), 2.3 (retiro seguro del prototipo heredado — ver regla 9), 3A (conexión a Neon `demo`, migración inicial, seed real), 3B.1 (autenticación backend profesional: login, refresh con rotación, autorización por rol, administración de usuarios, bootstrap del primer admin construido y testeado pero nunca ejecutado) y 3B.2 (corrección del modelo de credenciales: usuario+contraseña reemplazado por selección de identidad + PIN numérico individual de 4 dígitos, con bloqueo persistente por intentos fallidos; `production` sigue sin tocarse; sigue sin ejecutarse el bootstrap del admin). Sin pantalla de login ni ningún cambio funcional en el frontend todavía.
