...
**Actualización Etapa 5B (sin commit):** frontend operativo de Stock implementado y auditado sobre el contrato real de 5A: inventario con filtros y paginación server-side, detalle e historial inmutable, ingresos/consumos para todo usuario autenticado, fechas retroactivas y ajustes solo para `ADMIN`, y administración de categorías/productos exclusiva de `ADMIN`. No incluye reportes, compras, CRUD de destinos, backend nuevo ni migraciones.

Ver `docs/MIGRATION_PLAN.md` para la etapa vigente. El proyecto completó las Etapas 0–5A; la Etapa 5B está terminada y auditada en el árbol de trabajo, pendiente de revisión/commit humano. `docs/UI_CONTEXT.md` sigue siendo la fuente de verdad visual. `production` continúa sin tocarse; **el primer administrador de `demo` ya fue creado por el usuario personalmente** con el bootstrap interactivo — no se vuelve a ejecutar; los 4 empleados reales siguen `PENDING_ACTIVATION`, sin PIN, y no se activan ni se les asigna PIN sin autorización explícita puntual. Las pruebas de 5B usan respuestas sintéticas y no modifican filas reales. No hay dashboard ni otros módulos nuevos (reales o mock). Para módulos nuevos: conservar los emojis del prototipo junto con texto accesible.
