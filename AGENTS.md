...
**Actualización Etapa 5A (sin commit):** backend base de Stock implementado y auditado: consulta de catálogo/historial, administración de categorías y productos, e ingresos/consumos/ajustes transaccionales y auditados. No incluye frontend, reportes, compras ni CRUD de destinos.

Ver `docs/MIGRATION_PLAN.md` para la etapa vigente. El proyecto completó las Etapas 0–4B; la Etapa 5A está terminada en el árbol de trabajo y pendiente de revisión/commit humano. `docs/UI_CONTEXT.md` sigue siendo la fuente de verdad visual. `production` continúa sin tocarse; **el primer administrador de `demo` ya fue creado por el usuario personalmente** con el bootstrap interactivo — no se vuelve a ejecutar; los 4 empleados reales siguen `PENDING_ACTIVATION`, sin PIN, y no se activan ni se les asigna PIN sin autorización explícita puntual. Las filas reales del seed no se modifican en tests: las integraciones usan fixtures sintéticas con limpieza y verificación de conteos. No hay frontend de Stock, dashboard ni otros módulos nuevos (reales o mock). Para módulos nuevos: conservar los emojis del prototipo junto con texto accesible.
