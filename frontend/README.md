...
## Datos y caché (Etapa 5P)

Toda lectura del backend pasa por TanStack Query: claves en `src/api/queryKeys.ts` (siempre con el id del usuario), política por defecto en `src/api/queryClient.ts` (`STALE_TIME` operativo/métricas/catálogo) y alcance en `src/api/useSessionScope.ts`. Reglas: navegación interna solo con `Link`/`NavLink`; nunca storage persistente; revalidación sin borrar datos (`PageHeader refreshing`); invalidar solo los dominios afectados tras una mutación. Los tests montan componentes con `src/test/render.tsx` (un `QueryClient` nuevo por caso; opción `route` para pantallas con `NavLink`). `src/test/navigationPerformance.test.tsx` fija el presupuesto de navegación sobre la app completa. Detalle en `docs/ARCHITECTURE.md` §22.
