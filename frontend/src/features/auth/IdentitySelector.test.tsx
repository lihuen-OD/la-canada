...
describe('IdentitySelector', () => {
  ...
  it('distingue al administrador visual y semánticamente', async () => {
    ...
    expect(screen.getByLabelText('Cuenta de administrador')).toBeInTheDocument();
  ...
  it('usa colorHex de forma segura: un valor null cae a un color neutro sin romper el render', async () => {
    ...
    const card = await screen.findByRole('button', { name: /administrador/i });
    expect(card).toBeInTheDocument();
    expect(card.style.getPropertyValue('--identity-card-color')).toBe('#8a8f8a');
  });
